---
title: Guild Raid Timeline
type: timeline
aliases:
- Guild Raids Timeline
categories:
- Timelines
source:
  title: Guild Raid Timeline
  url: https://eq2.fandom.com/wiki/Guild_Raid_Timeline
  history: https://eq2.fandom.com/wiki/Guild_Raid_Timeline?action=history
  revision: 1721517
  revised: '2023-04-23T22:20:04Z'
  license: CC BY-SA 3.0
---

*See: Guild Writs for more information.*

## Guild Status Raids

Raids are given as a reward for achieving guild levels and are intended as a training ground for new raiders.  Each raid is Group x2, and will scale according to the higest level character in your raid.  In order to advance to the second raid, you must complete the first. To get to the third, you must complete the second, etc. After completing all five new raids, you will be able to do them in any order.  Not all members in the raid need be of the same guild.
![The choices seen for guild writ raids when all raids have been unlocked.](images/Guildwritraids.jpg)
The raids are designed as follows:

- Lvl 5  [[The Shattered Vale: A Bold Confrontation]] Add Management (NPCs spawn every X seconds and attack raid members)
- Lvl 10 [[The Eternal Gorge: A Daring Confrontation]] Aggro Management (Hate list shuffling or adding hate for subclass type)

- Lvl 15 [[The Firemyst Gully: A Gallant Confrontation]] AE Stun / Stifle (2-3 sec duration)

- Lvl 20 [[The Bloodskull Valley: A Noble Confrontation]] Resistance Fight (NPC uses powerful AE of type X)
- Lvl 25 [[The Cove of Decay: A Heroic Confrontation]] AE Charm (become the pet of the enemy!)

- Lvl 30 [[The Unkempt Vale: A Lordly Confrontation]]

Rewards for defeating the bosses within each raid will vary based on your level range.
Up to level 50, you will receive rare harvestable items, recipe books, and spell scrolls.
At level 50, 60, and 70 molds and patterns may drop off boss creatures.These patterns will be for very light, light, medium, or heavy armor. Examining the patterns will give you a quest, and upon turning it in to a Quartermaster, you will receive a piece of armor.
To begin, get two groups together into a raid and speak with the City Registrar Glamis{{waypoint -84, -37, -2}} in East Freeport or Royal Accountant Fowler in South Qeynos (563, -10, 164).  This raid will scale to the lowest level that the groups are mentored down to, so make the proper adjustments to the raid level prior to clicking on the door and porting into the instance.  Have the group member with the wanted quest, whether that be the first in the series or the third, be the one to click on the door to port your raid members into the wanted zone.

## Links and Sources

[http://forums.station.sony.com/eq2/posts/list.m?topic_id=346817](http://forums.station.sony.com/eq2/posts/list.m?topic_id=346817)
