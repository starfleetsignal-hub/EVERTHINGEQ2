---
title: Kleron's Wristguards
type: quest
expansion: Shattered Lands
level: '22'
difficulty: Solo
zone: '[[Nektulos Forest]]'
timeline: '[[Nektulos Forest Timeline]]'
journal_category: Nektulos Forest
starts: Speak to [[Kleron Asana]] in the camp near [[J'Rais Bridge]].  {{waypoint -300, 0, 138}}
next_quest: '[[Kleron''s Ring]]'
added_in: Shattered Lands
categories:
- Nektulos Forest Quests
- Nektulos Forest Solo Quests
- Quests
- Shattered Lands Quests
- Solo Quests
- Tier 3 Quests
- Tier 3 Solo Quests
source:
  title: Kleron's Wristguards
  url: https://eq2.fandom.com/wiki/Kleron's_Wristguards
  history: https://eq2.fandom.com/wiki/Kleron's_Wristguards?action=history
  revision: 1843765
  revised: '2024-12-06T02:40:04Z'
  license: CC BY-SA 3.0
---

This quest is part of a 6-quest series; the first five can be done in any order.

## Steps

1. Kill [[A dusk dart|dusk darts]] off the [[Port Naythex]] dock until you obtain 12 dusk dart glands. (904.29, -1.89, -136.66)
1. Visit [[Behemoth Pond]]. {{waypoint -208.98, 0.78, -257.73}}
1. Kill 18 skeletons. You can find level 20 skeletons west/south west of the [[Commonlands]] gate {{waypoint -82.91, 0.06, 233.29}}, 25 skeletons around [[The Funeral Pyre]], or higher level skeletons at [[Citadel of Gul'thex]]. You can find a closer set of skeletons at {{waypoint -90.55, -0.24, 201.67}}, just east of [[J'Rais Bridge]] in [[Nektulos Forest]].
1. Kill rumbleroots saplings until you obtain 5 pieces of rotting wood.  rumbleroots saplings can be found around {{waypoint -788, 66, -9}} and {{waypoint -853, 44, -127}}.
   - [[A rumbleroot|rumbleroots]] and [[A deadly rumbleroot|deadly rumbleroots]] do not count.
1. Return to [[Kleron Asana]] for your reward.

## Rewards

- One of the following, based on Class:
  - [[Acolyte's Wrought Wristguard]] (Clerics)
  - [[Bandit's Light Iron Vambraces]] (All Scouts)
  - [[Beguiler's Embroidered Cuffs]] (All Mages)
  - [[Expert's Fine Cloth Wrist Wraps]] (Brawlers)
  - [[Preserver's Cured Bands]] (Druids)
  - [[Sentry's Hammered Bracers]] (Crusaders, Warriors)
  - [[Soothsayer's Bound Chain Cuffs]] (Shamans)
