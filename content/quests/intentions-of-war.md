---
title: Intentions of War
type: quest
expansion: Destiny of Velious
expansion_source: patch
level: '96'
difficulty: Heroic
zone: '[[Kael Drakkel]]'
timeline: '[[Kael Drakkel Timeline]]'
starts: Speak to [[Corrin Ruadh]]
prerequisite: '[[The Ragebourne Identity]]'
next_quest: '[[Triumph in the Temple]]'
added_in: Destiny of Velious
achievement_xp: true
categories:
- AA Quests
- Destiny of Velious Quests
- Heroic Quests
- Kael Drakkel AA Quests
- Kael Drakkel Heroic Quests
- Kael Drakkel Quests
- Quests
- Tier 10 Heroic Quests
- Tier 10 Quests
source:
  title: Intentions of War
  url: https://eq2.fandom.com/wiki/Intentions_of_War
  history: https://eq2.fandom.com/wiki/Intentions_of_War?action=history
  revision: 1823525
  revised: '2024-09-28T11:23:03Z'
  license: CC BY-SA 3.0
---

## Steps

1. I must search for any information that might give some insight on the appearance of Drunder.
   - Collect Kromzek communiques
     - These are group updates; when one is clicked, everyone on the quest will receive the update
     - locations include:
       - Location to a Kromzek communique {{waypoint -413, 26, 1211}} (North-East room)
       - Location to a Kromzek communique {{waypoint -546, 24, 1284}} (North-East room)
       - Location to a Kromzek communique {{waypoint -408, 20, 1642}} (South-East room)
       - Location to a Kromzek communique {{waypoint -384, 20, 1744}} (South-East room)
       - Location to a Kromzek communique {{waypoint -97, -33, 1518}} (Throne room)
       - Location to a Kromzek communique {{waypoint 151, 24, 1619}} (South-West room)
       - Location to a Kromzek communique {{waypoint 189, 22, 1736}} (South-West room)
   - Inspect a Kromzek war map {{waypoint -392, 20, 1776}} (South-East room)
     - This is an individual update; each person on the quest must click on the map
1. Return to Corrin Ruadh {{waypoint -105.24, -10.81, 506.61}}

## Rewards

- At least 1p 50g

## Notes

After meeting with Corrin Ruadh he sends you to find six Kromzek Communiques and a Kromzek War Map. It is possible to invis to some of the communiques and the map, but be patient and on your guard, the Kromzek Praetorians do see through invisiblity.

The area of Kael Drakkel you will be looking for these in is the Hall of Legends map, and you will use the New Kael teleportal to get to the area you have to pass through to get here.

You may find two different spawn areas for the Communique in the same room as the war map,and you can stay and wait for respawn, respawns fairly quickly The following are just some of the locations.
