---
title: Whole Lotta' Holes
type: quest
expansion: Kingdom of Sky
expansion_source: patch
level: '58'
difficulty: Heroic
zone: '[[Tenebrous Tangle]]'
timeline: '[[Tenebrous Tangle Timeline]]'
journal_category: Tenebrous Tangle
starts: Examine [[A map (map of the Tenebrous Tangle)|a map]], received during the previous quest.
prerequisite: '[[Burglars Afoot]]'
added_in: Kingdom of Sky
aliases:
- Whole Lotta Holes
categories:
- Faction Quests
- Heroic Quests
- Kingdom of Sky Quests
- Quests
- Tenebrous Tangle Heroic Quests
- Tenebrous Tangle Quests
- The Lost Children of Marr (Faction)
- The Lost Children of Marr (Good Faction)
- Tier 6 Heroic Quests
- Tier 6 Quests
source:
  title: Whole Lotta' Holes
  url: https://eq2.fandom.com/wiki/Whole_Lotta'_Holes
  history: https://eq2.fandom.com/wiki/Whole_Lotta'_Holes?action=history
  revision: 1966775
  revised: '2026-02-01T13:48:11Z'
  license: CC BY-SA 3.0
---

## Steps

1. Find seven hiding holes, or "hydeeng hols", in [[Tenebrous Tangle]]. The holes look like the end of a barrel. Some are in the ground and some are in walls. Right click each barrel to update the quest. Although you can search for the holes in any order, one of the most efficient routes is as follows:
   - Smawlskail - [[Vicious Breeding Grounds]] {{waypoint 551, -146, 644}}
     - (Note: if you can't fly, use the teleport pad at ( 220, -226, 166 ) in the Hidden Refuge.  It is labelled Breeding Grounds, not Vicious Breeding Grounds.)
   - Steengstung - [[Bixie Isle (POI)|Bixie Isle]] {{waypoint -566, -52, -969}}
   - Antheel - [[Ravasect Incursion Point (POI)|Ravasect Incursion Point]] {{waypoint 514, 241, -666}}
   - Bludshot - [[Gazer Isle (POI)|Gazer Isle]] {{waypoint 967, 462, 172}}
   - Skwid - [[Fear Tainted Isle (POI)|Fear Tainted Isle]] {{waypoint -19, 241, 780}}
   - Berdkaij - [[Vultak Scavenging Site (POI)|Vultak Scavenging Site]] {{waypoint -761, 144, -91}}
   - Dyveeng Hydeeng - [[The Temple Grounds]] {{waypoint 222, 45, -94}}
1. Speak to [[Lady Erillis]] in the [[Hidden Refuge (POI)|Hidden Refuge]] to complete this quest.

## Rewards

- Your choice of:
  - [[Fire Etched Stone]]
  - [[Marrspin Tablet]]
- +10.000 faction with **The Lost Children of Marr**
