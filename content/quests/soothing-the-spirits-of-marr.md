---
title: Soothing the Spirits of Marr
type: quest
expansion: Shattered Lands
level: '12'
zone: '[[The Graveyard]]'
timeline: '[[Freeport Outlying Areas Timeline]]'
journal_category: The Graveyard
starts: Hail [[An Ethernere guard]] wandering around the zone
categories:
- Quests
- Shattered Lands Quests
- Solo Quests
- The Graveyard Quests
- The Graveyard Solo Quests
- Tier 2 Quests
- Tier 2 Solo Quests
source:
  title: Soothing the Spirits of Marr
  url: https://eq2.fandom.com/wiki/Soothing_the_Spirits_of_Marr
  history: https://eq2.fandom.com/wiki/Soothing_the_Spirits_of_Marr?action=history
  revision: 826594
  revised: '2018-01-23T16:07:12Z'
  license: CC BY-SA 3.0
---

*You may wish to do this quest while also doing [[The Book of the Dead (Quest)]], as both require you to kill graverobbers.*

## Notes

- Even though the Ethernere guards will attack Good-aligned players on sight, it is still possible to receive this quest by right-clicking and hailing the guard outside of their aggro radius.

## Steps

1. Kill graverobbers until you receive four relics.
   - Graverobbers only spawn during game night, between 8pm and 5am.
   - Graverobber spawn points *must* be empty of other mobs (skeletons and zombies); kill the placeholders just before 8pm game time so the next spawn will be a graverobber:
     - {{waypoint -85, -11, 28}}
     - {{waypoint -75, -15, 19}}
     - {{waypoint -61, -11, -32}}
     - {{waypoint 47, -3, -30}} - outside the blue quest area on the map but spawns graverobbers that update the quest.
   - Possible mobs are [[A novice graverobber]], [[An amateur graverobber]] and [[A graverobber]]; they can spawn as singles, duos or trios; and are all trackable.
1. Take the relics to the tomb of Sir Acknim {{waypoint -39, -10, -40}} and click on the tombstone in front of it.
1. Return to one of the Ethernere guards.

## Rewards

- At least 3s 62c
