---
title: If They Only Had a Brain
type: quest
expansion: Sentinel's Fate
expansion_source: chain
level: Scales
difficulty: Solo
zone: '[[Steamfont Mountains]]'
journal_category: World Event
starts: Speak With [[Blantsworth Coggington III]] just outside [[Gnomeland Security Headquarters]]
prerequisite: '[[Great Balls of Fire]]'
achievement_xp: true
events:
- Tinkerfest
categories:
- AA Quests
- Quests
- Scalable Quests
- Shattered Lands Quests
- Solo Quests
- Steamfont Mountains AA Quests
- Steamfont Mountains Quests
- Steamfont Mountains Solo Quests
- Tinkerfest
- World Event Quests
source:
  title: If They Only Had a Brain
  url: https://eq2.fandom.com/wiki/If_They_Only_Had_a_Brain
  history: https://eq2.fandom.com/wiki/If_They_Only_Had_a_Brain?action=history
  revision: 1877287
  revised: '2025-06-12T00:32:06Z'
  license: CC BY-SA 3.0
---

## Notes

- The Tinkering skill is NOT required for this quest.  You must have done [[Great Balls Of Fire]] at least 5 times for the questgiver to offer this quest.

## Steps

1. Scribe the [[Tinkered Clockwork Brain Blueprints]] recipe (Artisan level 2) you receive, and craft five [[Tinkered Clockwork Brains]] (Tinkering skill 50).
   - Each combine requires 1 lead cluster, 2 tin clusters, and 2 Basic Coal. Produces 1-2 brains. There is a broker in the Inn if you came unprepared with materials.
   - These items can be made by someone without the Tinkering skill (the lowest level complete still produces a brain), although you won't be able to counter any events.
   - Mass Production does not help but the recipe creates 2 if you complete all four bars and both of those count toward the 5 brains required.
1. Install five [[Tinkered Clockwork Brains]] into [[Erratic Tinkerfest Clockwork|Erratic Tinkerfest Clockworks]].
   - Target an [[Erratic Tinkerfest Clockwork]] outside of Gnomeland Security, and use the [[Coggington's Clockwork Incapacitator]] that you received. Then click the Incapacitated Erratic Tinkerfest Clockwork to Install a Brain (or right-click and choose *Install The Brain*). Do this quickly as they won't be incapacitated for long.
1. After installing the last brain, lead a Cognitive Tinkerfest Clockwork back to [[Blantsworth Coggington III]].

## Rewards

- 5 Jubilation Medals
- One Of The Following:
  - [[Corroded Clockwork Beetle]]
  - [[Corroded Clockwork Grappler Beetle]]
  - 5 [[Shiny Tinkerfest Cog]]
  - [[Call Prototype Cognitive Clockwork|Prototype Cognitive Clockwork]] spell
  - [[Rusty Buzzblade]]
  - [[Rusty Illuminated Polearm]]
  - [[Rusty Mechanized Blade]]
  - [[Rusty Ratchet Wrench]]
  - [[Rusty Sonic Drill]]

- After finishing this quests 5 times total you're able to buy the following title from Myron:
  - [[Title: Brainiac]] (new in 2015)

## Thoughtful Tips For Top-Notch Tinkerfest Times!

![Sample "If They Only Had A Brain" Macro](images/Brain_Macro.png)

- Make a macro for targeting and incapacitating the erratic tinkerfest clockworks. You merely have to be near one, press the button, then right-click to install the new brain.
  1. Press **O** to open the Macros window.
  1. Find a blank macro slot and click on **<click to edit>**
  1. Add a Command Step: **/target_nearest_npc**
  1. Add a Use Item step, then drag the Coggington's Clockwork Incapacitator to it.
  1. Give your macro a title.
  1. Click **OK**
  1. Drag the new macro to a hotbar for easy access.
