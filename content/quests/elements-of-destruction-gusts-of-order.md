---
title: 'Elements of Destruction: Gusts of Order'
type: quest
expansion: Chaos Descending
level: '110'
difficulty: Heroic
zone: '[[Myrist, the Great Library]]'
timeline: '[[Chaos Descending Timeline]]'
journal_category: Signature
starts: Click on a corrupted conflagration of air at {{waypoint 251, -7, -19}}
prerequisite: '[[Elements of Destruction: Shadow Casting in the Dark]]'
next_quest: '[[Elements of Destruction: Face the Chaos]]'
added_in: Chaos Descending
categories:
- Chaos Descending Quests
- Heroic Quests
- Myrist, the Great Library Heroic Quests
- Myrist, the Great Library Quests
- Quests
- Signature Quests
- Tier 12 Heroic Quests
- Tier 12 Quests
source:
  title: 'Elements of Destruction: Gusts of Order'
  url: https://eq2.fandom.com/wiki/Elements_of_Destruction:_Gusts_of_Order
  history: https://eq2.fandom.com/wiki/Elements_of_Destruction:_Gusts_of_Order?action=history
  revision: 2018022
  revised: '2026-07-16T16:42:34Z'
  license: CC BY-SA 3.0
---

## Notes

![](images/Inert_Eryslai_Purifying_Rune.png)

- ***You may not receive all of the [[Ephemeral gust|ephemeral gusts]] needed for this quest on one run through the zone.***

- The quest is listed as heroic but can be soloed
- You receive an [[Inert Eryslai Purifying Rune]] when you click on the corrupted air to start the quest.

## Steps

![](images/Ephemeral_gust.png)

1. Take the teleporter at {{waypoint 260, -20, -285}} to the [[Elemental Portal Gallery]]. Use the wind portal at {{waypoint 714, 412, -380}} to enter [[Eryslai: The Midnight Aerie (Solo)|Eryslai: The Midnight Aerie [Solo]]].
1. Progress through the zone. (See [[Eryslai: The Midnight Aerie (Solo)|Eryslai: The Midnight Aerie [Solo]]] for strategies).
1. Kill zephyren, phoenix, nilborien and wilders to obtain 19 [[Ephemeral gust|ephemeral gusts]].
   - The drops are uncommon. One is dropped for each party member.
   - *Note: It can happen that you don't get all 19 drops from 1 run of the zone, so you have to reenter a fresh version of the instance to get the final drop and be able to complete the quest.*
1. Use the [[Inert Eryslai Purifying Rune]] from the quest helper to recieve the [[Imbued Eryslai Purifying Rune]]
1. After killing the final boss, [[Sterek Swiftwind]], *Do NOT take the portal back to the Library*. Take the **Portal to Oeranthia Spire** at 1200, 324 -278.
   - Note: If you have enough drops, and the portal doesn't appear, examine the [[Inert Eryslai Purifying Rune]] from your inventory.
   - Note: If you didn't get enough drops the first run through the zone, the portal might not appear on the second run. In that case, you can drop down to Oeranthia Spire using Featherfall. Use the waypoint to make sure you aim for the right island. It's in front of, and far below, where the portal should be. Once you get the update, there's no way to return to Myrist, so you have to use an alternative transportation such as Evac or call to home city.
1. Click on **The Oeranthia Breathstone**![The Oeranthia Breathstone](images/TheOeranthiaBreathstone.jpg) at {{waypoint 831, -290, -469}}.
1. Now you can take the portal that spawns and return to the Library.

## Rewards

- [[Oeranthia Champion Accolade]]
- [[Zephyren Feather Bracelet]]
- [[Gustrider's Sureshot Crossbow]]
- At least 1p 41g 75s 48000 status
