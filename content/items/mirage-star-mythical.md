---
title: Mirage Star (Mythical)
type: item
item_kind: Equipment
icon: images/Item_3723.png
tier: Mythical
level: '80'
slot: Primary
classes: '[[Illusionist]]'
flags: lore no-trade no-value
stats:
  sta: '+54'
  int: '+54'
  crit: '3.1'
  critbonus: '1.1'
  potency: '5.1'
  dmg: 181 - 544
  delay: '6.0'
  drating: '120.96'
  dtype: Main Hand Crushing
  wtype: Staff
effect_name: Rapid Strikes, Soothing Mind, Personae Mastery
effects: "- When Equipped:\n  - Changes Rapidity IV to a group buff and adds 10% double attack.\n- When Equipped:\n  - On a hostile spell cast this spell has a 12% chance to cast Soothing Mind on the group.  Lasts for 15.0 seconds.\n    - On any combat or spell hit this spell will cast Soothing Mind on the group.  Lasts for 15.0 seconds.\n      - Increases power of target by 360.\n      - Grants a total of 1 trigger of the spell.\n    - Cannot be modified except by direct means\n  - Cannot be modified except by direct means\n- When Equipped:\n  - Increases the max health of the illusionist's Personae Reflection VII by 25%"
obtained_from: '[[The Mirage Star]]'
item_link: \aITEM 529211491 -117008505:Mirage Star\/a
categories:
- Ability Doublecast (Equipment)
- Census Credits
- Combat Skills (Equipment)
- Crit Bonus (Equipment)
- Crit Chance (Equipment)
- Epic Weapons
- Equipment
- Illusionist Equipment
- Main Hand Crushing (Equipment Type)
- Mythical Equipment
- Personae Mastery (Equipment)
- Potency (Equipment)
- Primary (Inventory Slot)
- Rapid Strikes (Equipment)
- Soothing Mind (Equipment)
- Staff (Weapon Type)
- Tier 9 Equipment
source:
  title: Mirage Star (Mythical)
  url: https://eq2.fandom.com/wiki/Mirage_Star_(Mythical)
  history: https://eq2.fandom.com/wiki/Mirage_Star_(Mythical)?action=history
  revision: 1902022
  revised: '2025-09-18T21:18:46Z'
  license: CC BY-SA 3.0
expansion: Rise of Kunark
expansion_source: source
---

## Notes

Personae Mastery only affects the spell Personae Reflection VII as of 10-20-2009.  Personae Reflection I through VI are not included in concentration reduction.
