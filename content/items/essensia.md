---
title: Essensia
type: item
item_kind: Equipment
icon: images/Item_2848.png
tier: Legendary
level: '60'
slot: Primary
classes: ', , , , [[Channeler]]'
flags: attunable heirloom
stats:
  str: '+31'
  sta: '+31'
  crit: '1.2'
  critbonus: '0.3'
  potency: '1.7'
  dps: '5.7'
  dmg: 63 - 188
  delay: '4.0'
  drating: '62.59'
  dtype: One-Handed Slashing
  wtype: Sword
effect_name: Soul Rend
effects: "- Applies Soul Rend when Equipped.\n  - On a combat hit this spell may cast Soul Rend on target of attack.  Triggers about 1.8 times per minute.\n    - Decreases power of target by 191.\n    - Increases power of caster by 190.\n    - Cannot be modified except by direct means"
obtained_from: '[[Infiltrating the Sanctum: Shadow Ordinance]]'
item_link: \aITEM 1865328685 -109021470:Essensia\/a
image: images/Essensia.jpg
aliases:
- Essencia
categories:
- Assassin Equipment
- Bard Equipment
- Beastlord Equipment
- Berserker Equipment
- Brigand Equipment
- Census Credits
- Channeler Equipment
- Combat Skills (Equipment)
- Crit Bonus (Equipment)
- Crit Chance (Equipment)
- Crusader Equipment
- DPS (Equipment)
- Dirge Equipment
- Druid Equipment
- Equipment
- Equipment pages with existing iname images
- Fury Equipment
- Guardian Equipment
- Katana (Equipment Type)
- Legendary Equipment
- One-Handed Slashing (Equipment Type)
- Paladin Equipment
- Potency (Equipment)
- Predator Equipment
- Primary (Inventory Slot)
- Ranger Equipment
- Rogue Equipment
- Scout Equipment
- Secondary (Inventory Slot)
- Shadowknight Equipment
- Soul Rend (Equipment)
- Swashbuckler Equipment
- Sword (Weapon Type)
- Tier 7 Equipment
- Troubador Equipment
- Warden Equipment
- Warrior Equipment
source:
  title: Essensia
  url: https://eq2.fandom.com/wiki/Essensia
  history: https://eq2.fandom.com/wiki/Essensia?action=history
  revision: 1897178
  revised: '2025-09-18T18:29:41Z'
  license: CC BY-SA 3.0
expansion: Kingdom of Sky
expansion_source: source
---

PROTIP: while quest is one per character, the weapon is HEIRLOOM, so an alt can complete quest for dual set.
