---
title: Eye of the Siren (Fabled)
type: item
item_kind: Equipment
icon: images/Item_3720.png
tier: Fabled
level: '80'
slot: Primary
classes: '[[Coercer]]'
flags: no-transmute  no-salvage  lore no-trade no-value
stats:
  sta: '+48'
  int: '+48'
  crit: '2.6'
  critbonus: '0.9'
  potency: '3.9'
  dmg: 164 - 492
  delay: '6.0'
  drating: '109.44'
  dtype: Main Hand Crushing
  wtype: Staff
effect_name: Siren's Gift, Invoke Leandre
effects: "- When Equipped:\n  - Reduces the reuse timers of all spells by 5%.\n  - On a damage spell hit this spell has a 9% chance to cast Siren's Gift on the group.  Lasts for 10.0 seconds.\n    - Reduces the power cost of group members' spells by 10%.\n    - Cannot be modified except by direct means\n  - Cannot be modified except by direct means\n- Applies Invoke Leandre when Activated."
obtained_from: '[[Leandre''s Shard: Drusella''s Extraction]]'
item_link: \aITEM 1658623060 -1917028012:Eye of the Siren\/a
categories:
- Ability Doublecast (Equipment)
- Census Credits
- Coercer Equipment
- Combat Skills (Equipment)
- Crit Bonus (Equipment)
- Crit Chance (Equipment)
- Epic Weapons
- Equipment
- Fabled Equipment
- Invoke Leandre (Equipment)
- Main Hand Crushing (Equipment Type)
- Potency (Equipment)
- Primary (Inventory Slot)
- Siren's Gift (Equipment)
- Staff (Weapon Type)
- Tier 9 Equipment
source:
  title: Eye of the Siren (Fabled)
  url: https://eq2.fandom.com/wiki/Eye_of_the_Siren_(Fabled)
  history: https://eq2.fandom.com/wiki/Eye_of_the_Siren_(Fabled)?action=history
  revision: 1897378
  revised: '2025-09-18T18:36:22Z'
  license: CC BY-SA 3.0
expansion: Rise of Kunark
expansion_source: source
---


